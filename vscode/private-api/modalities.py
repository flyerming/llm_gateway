...
# >>> THE ONLY LINES THAT DIFFER FROM `codexcli/private-api/modalities.py` <<<
# This copy is the vscode one, so `_toolkits.vscode` in `model-config.jsonc` is
# what applies. Declared here rather than threaded through every call site:
# a default argument that silently means "codexcli" in a vscode install is
# exactly the kind of thing that gets forgotten.
#
# Everything below this block must stay identical to the codexcli copy --
# `diff codexcli/private-api/modalities.py vscode/private-api/modalities.py`
# should show this comment and nothing else.
